// scripts/match-scene.ts
//
// 本地测试专用:与生产 app/api/enhance-photo/match-scene.ts 逻辑一致,
// 但类型签名绑定到 scripts/tag-user-photo.ts 的 UserPhotoTags,
// 避免走 @/* 路径别名和 scanner 模块依赖。

import type { UserPhotoTags } from './tag-user-photo';
...
function scoreScene(userTags: UserPhotoTags, scene: SceneEntry): number {
  let score = 0;
